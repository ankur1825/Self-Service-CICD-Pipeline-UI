...
function PipelineForm() {
    ...
    //     if (name === "sonarQubeEnabled" || name === "opaScanEnabled" || name === "trivyScanEnabled") {
        ...
        const id = setInterval(async () => {
            try {
                ...
                if (result.building === false) {
                    clearInterval(id);
                    setIntervalId(null);
                    setFormDisabled(false); // 🛠 Re-enable form here!
                }
            } catch (err) {
                console.error('Error fetching logs:', err);
                clearInterval(id);
                setIntervalId(null);
                setFormDisabled(false); // 🛠 Even if error, re-enable form
            }
        }, 3000);
        setIntervalId(id);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            ENABLE_TRIVY: formData.trivyScanEnabled === true  // Added
        };
        ...
        try {
            ...
            if (check.status === "Build triggered") {
                ...
                setBuildNumber(1);
                setShowLogs(true);
                startPollingLogs(formData.project_name, 1);
                return;
            }
                ...
                {showPipelineFields && (
                    <>
                        ...
                        <FormControlLabel
                            control={
                                <Switch
                                    checked={formData.trivyScanEnabled}
                                    onChange={handleSwitchChange}
                                    name="trivyScanEnabled"
                                />
                            }
                            label={formData.trivyScanEnabled ? "Trivy Scan Enabled" : "Trivy Scan Disabled"}
                            style={{ marginBottom: '16px' }}
                        />
                    </>
                )}
